-- ═══════════════════════════════════════════════════════════════════
-- TOWNSHUB PROPERTY MODULE — DEMO DATA SEED
...
BEGIN
  ...
  IF v_tid IS NULL THEN
    RAISE EXCEPTION 'No tenant named "Townshub" found. Update the WHERE clause to match your tenant name.';
  END IF;
  ...
  INSERT INTO units (id, tenant_id, property_id, owner_id, unit_number, floor, type,
    area_sqm, bedrooms, bathrooms, parking_spaces, furnished, status, market_rent, features)
  VALUES
    (gen_random_uuid(), v_tid, v_p1, v_own1, 'PH-01', 4, 'penthouse', 150.00, 3, 2, 2, 'furnished',  'vacant',   1800.00, ARRAY['360° Views','Private Pool','Smart Home','Wine Cellar'])
  ...
  -- ── LEASES — insert as draft, then activate (triggers rent schedule + unit status) ──
  ...
  VALUES
    (gen_random_uuid(), v_tid, v_u103, v_r3, v_own1,
     ...
     'Utilities included up to €150/month. Excess charged to tenant.',
     'Consider early renewal offer — Michael expressed interest in staying long-term.',
     TRUE, 12)
  ...
  VALUES
    (gen_random_uuid(), v_tid, v_uO1, v_r5, v_own2,
     ...
     'TechCorp requested signage rights — approved by owner.',
     FALSE, 12)
  ...
  VALUES
    (gen_random_uuid(), v_tid, v_uS2, v_r7, v_own3,
     ...
     'MediaGroup is a fast-growing agency — likely to need more space within 12 months.',
     TRUE, 24)
  ...
  VALUES
    (gen_random_uuid(), v_tid, v_uS4, v_r8, v_own3,
     ...
     'Permitted use: warehousing and distribution. Loading dock access 6am–10pm only.',
     FALSE, 12)
  RETURNING id INTO v_l8;

  -- ── ACTIVATE LEASES (triggers rent schedule generation + unit status update) ──
  ...
  -- ── MARK HISTORICAL RENT SCHEDULE ENTRIES AS PAID/OVERDUE ────────
  ...
  -- Current month pending (TechCorp and Logistics — awaiting SEPA)
  UPDATE rent_schedule SET status = 'pending'
  WHERE lease_id IN (v_l5, v_l8)
    AND due_date >= DATE_TRUNC('month', CURRENT_DATE)
    AND due_date < DATE_TRUNC('month', CURRENT_DATE) + INTERVAL '1 month';

  -- ── PROPERTY PAYMENTS (receipts for paid entries) ─────────────────
  ...
  INSERT INTO property_payments (tenant_id, lease_id, property_tenant_id, unit_id,
    amount, payment_type, method, reference, payment_date, notes)
  VALUES
    (v_tid, v_l1, v_r1, v_u101, 1700.00, 'deposit', 'bank_transfer', 'DEP-L1-2024', CURRENT_DATE - INTERVAL '8 months', 'Security deposit — Lucas Fernandez'),
    (v_tid, v_l3, v_r3, v_u103, 3600.00, 'deposit', 'bank_transfer', 'DEP-L3-2024', CURRENT_DATE - INTERVAL '10 months', 'Security deposit — Michael Georgiou (3 months)'),
    (v_tid, v_l4, v_r4, v_uVA,  6600.00, 'deposit', 'bank_transfer', 'DEP-L4-2025', CURRENT_DATE - INTERVAL '4 months', 'Security deposit — Sofia Petridou (3 months)');
  ...
  VALUES
    (v_tid, v_u105, v_p1, NULL, 'manager', NULL, 'plumbing',
     'Bathroom leak — Unit 105', 'Water leak from shower tray seeping into floor below. Reported during routine inspection. Unit currently empty.',
    ...
    (v_tid, v_u103, v_p1, v_l3, 'tenant', v_r3, 'hvac',
     'AC not cooling — Unit 103', 'Air conditioning unit in master bedroom not producing cold air. Tenant reports issue since 3 days ago.',
    ...
    (v_tid, v_uVB, v_p2, NULL, 'manager', NULL, 'security',
     'Front door lock mechanism faulty — Villa B', 'Electronic lock not engaging properly. Temporary manual lock installed. Locksmith quote requested.',
    ...
    (v_tid, v_uS3, v_p3, NULL, 'manager', NULL, 'electrical',
     'Electrical fault — Retail 01', 'Main circuit breaker tripping repeatedly. Building electrician inspected and found faulty wiring in distribution board.',
    ...
    (v_tid, NULL, v_p2, NULL, 'manager', NULL, 'garden',
     'Garden landscaping — Palm Court Common Areas', 'Seasonal pruning and replanting of common garden areas. Scheduled as part of Q2 maintenance plan.',
    ...
    (v_tid, v_u101, v_p1, v_l1, 'tenant', v_r1, 'appliance',
     'Dishwasher not draining — Unit 101', 'Dishwasher leaves standing water after cycle. Tenant has been using it since move-in.',
  ...
  VALUES
    (v_tid, v_own1,
     ...
     'Units 101, 102, 103 — 2 months ago. Dishwasher repair deducted.'),
    ...
    (v_tid, v_own2,
     ...
     'Villa A + Office 1 — all rents collected. Garden maintenance billed separately.'),
  ...
  VALUES
    (v_tid, v_p1, v_u101, v_l1, v_r1, 'lease_agreement',
     'Lease Agreement — Lucas Fernandez (Unit 101)',
    ...
    (v_tid, v_p1, v_u102, v_l2, v_r2, 'lease_agreement',
     'Lease Agreement — Anna Constantinou (Unit 102)',
    ...
    (v_tid, v_p1, v_u103, v_l3, v_r3, 'lease_agreement',
     'Lease Agreement — Michael Georgiou (Unit 103)',
    ...
    (v_tid, v_p2, v_uVA, v_l4, v_r4, 'lease_agreement',
     'Lease Agreement — Sofia Petridou (Villa A)',
  ...
  VALUES
    (v_tid, v_p3, v_own3, 'insurance',
     'Building Insurance — Ocean View Commercial Centre',
